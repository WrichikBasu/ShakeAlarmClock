	...
	public Dialog onCreateDialog(@Nullable Bundle savedInstanceState) {
		...
		AlertDialog.Builder builder = new AlertDialog.Builder(requireContext())
			...
			.setPositiveButton(R.string.batteryOptim_positive, (dialog, which) -> {
				if (attached && android.os.Build.VERSION.SDK_INT >=
			...
			.setNeutralButton(R.string.batteryOptim_neutral, (dialog, which) -> {
				...
				requireContext().getSharedPreferences(
						ConstantsAndStatics.SHARED_PREF_FILE_NAME, Context.MODE_PRIVATE)
					.edit()
					.remove(ConstantsAndStatics.SHARED_PREF_KEY_SHOW_BATTERY_OPTIM_DIALOG)
					.putBoolean(
						ConstantsAndStatics.SHARED_PREF_KEY_SHOW_BATTERY_OPTIM_DIALOG,
						false)
					.commit();
			});
